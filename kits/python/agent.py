...
@dataclass
class GameState:
    """Tracks persistent game state across turns and matches"""
    relic_positions: List[np.ndarray]
    discovered_relics: Set[int]
    explore_locations: Dict[int, Tuple[int, int]]

class Agent:
    def __init__(self, player: str, env_cfg) -> None:
        """Initialize agent with team info and game state tracking"""
        self.player = player
        self.team_id = int(player[-1])  # Convert player_0/player_1 to 0/1
        self.opp_team_id = 1 - self.team_id
        self.env_cfg = env_cfg
        np.random.seed(0)
        
        # Initialize persistent game state
        self.state = GameState([], set(), {})
    ...
    def get_unit_action(self, unit_id: int, unit_pos: np.ndarray, 
                       step: int) -> List[int]:
        """Determine action for a single unit based on game state"""
        if self.state.relic_positions:
            # Move towards/around first discovered relic
            relic_pos = self.state.relic_positions[0]
            dist = abs(unit_pos[0] - relic_pos[0]) + abs(unit_pos[1] - relic_pos[1])
            
            if dist <= 4:  # Hover around relic
                return [np.random.randint(0, 5), 0, 0]
            return [direction_to(unit_pos, relic_pos), 0, 0]
        
        # Explore random location, updating every 20 steps
        if step % 20 == 0 or unit_id not in self.state.explore_locations:
            self.state.explore_locations[unit_id] = (
                np.random.randint(0, self.env_cfg["map_width"]),
                np.random.randint(0, self.env_cfg["map_height"])
            )
        return [direction_to(unit_pos, self.state.explore_locations[unit_id]), 0, 0]
